...
base_url = 'http://umuryango.rw/'


def get_news_links(page_url):
    page = requests.get(page_url)
    soup = BeautifulSoup(page.content, 'html.parser')
    news_articles = soup.find_all('h2', class_='artticle')
...
def next_page_url(base, count):
    return f'{base}/?debut_ART={count}#pagination_ART'
...
def get_article_content(url, category='news'):
    ...
    soup = BeautifulSoup(page.content, 'html.parser')
    text = ''
    if(soup.find('h1', class_='artticle') is None):
        article_title = 'N/A'
    else:
        article_title = soup.find(
            'h1', class_='artticle').getText()
    if(soup.find('div', class_='wrapart') is None):
        text = ''
    else:
        article_paragraphs = soup.find(
            'div', class_='wrapart').find_all('p')
        for p in article_paragraphs:
            text += p.getText() + '\n'
    print({'Title': article_title, 'Body': text, 'Url': url, 'Category': category})
...
get_article_content(
    'http://umuryango.rw/imyidagaduro/article/kayitare-wayitare-dembe-yatanze-umusanzu-we-yerekana-uburyo-bwiza-bwizewe-bwo')
# get_news_links('http://umuryango.rw/imyidagaduro/')
# fetch_as_much_links(ent_articles, 200, 'umuryango_ent_articles.txt')
# create_csv_file('umuryango_ent_articles.txt',
#                 'umuryango_ent_articles.csv', 'Entertainment')
